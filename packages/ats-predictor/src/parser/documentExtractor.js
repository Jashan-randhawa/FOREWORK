...
export class DocumentExtractor {
   ...
   * @param {string} url
   * @param {Object} [cloudinaryClient]
   * @returns {string[]}
   */
  static getCloudinaryCandidates(url, cloudinaryClient = null) {
    if (!url || typeof url !== "string" || !url.includes("res.cloudinary.com")) {
      return [];
    }

    const client = cloudinaryClient || (typeof globalThis !== "undefined" && globalThis.cloudinary);
   ...
   * @param {Object} [options.cloudinaryClient]
   * @returns {Promise<{ buffer: Buffer, mimeType: string, filename: string }>}
   */
  static async fetchFromUrl(url, options = {}) {
    if (!url || typeof url !== "string") {
      throw new AppError("Invalid document URL provided", 400);
    }

    const client = options.cloudinaryClient || (typeof globalThis !== "undefined" && globalThis.cloudinary);
    ...
    const urlsToTry = [
      ...this.getCloudinaryCandidates(url, client),
      url,
    ];
